...
export default function slackin ({
  ...
  if ( typeof interval === 'undefined' ) throw new Error('Must provide an `interval`.')
  ...
  if ( path.length > 1 ) {
    // static files
    app.use(path.replace(/\/$/, '') + '/assets', express.static(assets))
  }
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (domain.match(/^([a-z]{2}\.)?ibm\.com$/)) {
      return res
        .status(303)
        .json({ msg: 'IBMers should join the slack team directly. Redirecting... ',
          redirectUrl: 'https://gameontext.slack.com/signup' })
    }
    if (domain === 'redhat.com') {
      return res
        .status(303)
        .json({ msg: 'Red Hatters should join the slack team directly. Redirecting... ',
          redirectUrl: 'https://gameontext.slack.com/signup' })
    }
